...
function renderStreams() {
  ...
  app.innerHTML = `<section class="panel">
    ...
    <div class="stream-grid">${content.streams.map((stream) => `<article class="stream-card"><h3>${escapeHtml(stream.title)}</h3><p>${escapeHtml(stream.description)}</p><p class="details">${escapeHtml(stream.details)}</p><div class="video-links">${(stream.videos || [stream.video]).filter(Boolean).map((resource) => `<a class="video-link" href="${escapeHtml(resource.url)}" target="_blank" rel="noopener">${escapeHtml(resource.type || 'Watch')}: ${escapeHtml(resource.label)} <span aria-hidden="true">↗</span></a>`).join('')}</div></article>`).join('')}</div>
