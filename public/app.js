...
const stepNames = ['Streams', 'Topics', 'Team style', 'Preferences', 'Review'];
...
function renderWelcome() {
  ...
  app.innerHTML = `
    <section class="hero">
      ...
      <h1>Find your project direction—and your team.</h1>
      <p class="lead">Explore three approaches to BIOB90, rank the topics that genuinely interest you, and complete a short teamwork quiz. Your responses will help us form groups around shared interests and complementary ways of working.</p>
      <form id="identity-form" class="form-card">
        ...
        <div class="field">
          <label for="student-number">Student number <span class="hint">Digits only. This lets you recover an autosaved response.</span></label>
          <input id="student-number" name="studentNumber" type="text" inputmode="numeric" autocomplete="off" pattern="[0-9]{8,12}" maxlength="12" required>
        </div>
...
function renderStreams() {
  ...
  app.innerHTML = `<section class="panel">
    <div class="section-heading"><div><div class="eyebrow">Step 1 of 5</div><h2>Choose how you want to investigate biology.</h2><p>Your stream determines the intellectual approach to your topic. Read all three descriptions before ranking them.</p></div><span class="count-badge">Rank all 3</span></div>
    <div class="stream-grid">${content.streams.map((stream) => `<article class="stream-card"><h3>${escapeHtml(stream.title)}</h3><p>${escapeHtml(stream.description)}</p><p class="details">${escapeHtml(stream.details)}</p></article>`).join('')}</div>
...
function renderTopics() {
  app.innerHTML = `<section class="panel">
    <div class="section-heading"><div><div class="eyebrow">Step 2 of 5</div><h2>Rank the five topics you most want to explore.</h2><p>Use the descriptions and starting sources to understand each topic. Your ranking should reflect genuine interest, regardless of your stream choices.</p></div><span class="count-badge">${record.topicRanking.length} of 5 selected</span></div>
  ...
  bindTopicControls();
  bindNavigation(() => record.topicRanking.length === 5 ? '' : 'Please select and rank exactly five topics.');
}
...
function quizResultHtml() {
  ...
  return `<article class="result-card"><div class="eyebrow">Your teamwork match</div><h3 class="result-name">${escapeHtml(outcome.name)}</h3><strong>${escapeHtml(outcome.tagline)}</strong><p>${escapeHtml(outcome.description)}</p><p class="hint"><strong>Watch-out:</strong> ${escapeHtml(outcome.watchOut)}</p>
...
function renderQuiz() {
  app.innerHTML = `<section class="panel">
    <div class="section-heading"><div><div class="eyebrow">Step 3 of 5</div><h2>${escapeHtml(content.quiz.title)}</h2><p>${escapeHtml(content.quiz.intro)}</p></div><span class="count-badge">${Object.keys(record.quizAnswers).length} of ${content.quiz.questions.length}</span></div>
    ...
    <p class="hint">This is an unofficial educational teamwork activity. Character names are used playfully and are not a psychological assessment.</p>
  </section>`;
...
function renderReview() {
  ...
  app.innerHTML = `<section class="panel">
    ...
    <div class="review-grid">
      ...
      <article class="review-card"><h3>Teamwork style</h3><strong>${escapeHtml(outcome?.name || 'Not complete')}</strong><p>${escapeHtml(outcome?.tagline || '')}</p></article>
...
function render() {
  ...
  renderProgress();
  ({ 1: renderStreams, 2: renderTopics, 3: renderQuiz, 4: renderPreferences, 5: renderReview })[step]();
}
...
async function renderAdminDashboard() {
  try {
    ...
    instructorContent.innerHTML = `<h2>Response export</h2><div class="review-grid"><div class="review-card"><span class="admin-stat">${summary.total}</span><br><span class="hint">started</span></div><div class="review-card"><span class="admin-stat">${summary.submitted}</span><br><span class="hint">submitted</span></div></div><p>Download the current spreadsheet-ready file. In-progress autosaves are included and clearly labelled.</p><p><a class="primary" href="/api/admin/submissions.csv">Download CSV</a></p><button id="admin-logout" class="text-button" type="button">Sign out</button>`;
