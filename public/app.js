...
function renderStreams() {
  ...
  app.innerHTML = `<section class="panel">
    ...
    <div class="stream-grid">${content.streams.map((stream) => `<article class="stream-card"><h3>${escapeHtml(stream.title)}</h3><p>${escapeHtml(stream.description)}</p><p class="details">${escapeHtml(stream.details)}</p><div class="video-links">${(stream.videos || [stream.video]).filter(Boolean).map((video) => `<a class="video-link" href="${escapeHtml(video.url)}" target="_blank" rel="noopener">Watch: ${escapeHtml(video.label)} <span aria-hidden="true">↗</span></a>`).join('')}</div></article>`).join('')}</div>
